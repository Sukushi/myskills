...
public class QuizController extends GenericController<QuizDto, QuizService> implements AliasableController<QuizDto> {
	...
	public ResponseEntity<Page<QuizDto>> findAllByThemeId(@PathVariable Long themeId, Pageable page) {
		return ResponseEntity.ok(service.findAllByThemeId(themeId,page));
	}
	
	@GetMapping("/name/{name}")
	public ResponseEntity<Page<QuizDto>> findAllByNameContaining(@PathVariable String name, Pageable page) {
		return ResponseEntity.ok(service.findAllByNameContaining(name,page));
	}
	
	@GetMapping("/name/{name}{status}")
	public ResponseEntity<Page<QuizDto>> findAllByNameContainingAndStatus(@PathVariable String name, @PathVariable QuizStatusEnum status, Pageable page) {
		return ResponseEntity.ok(service.findAllByNameContainingAndStatus(name,status,page));
	}
	
	@GetMapping("/fav/{userId}")
	public ResponseEntity<Page<QuizDto>> findAllFavoriteFromUser(@PathVariable Long userId, Pageable page) {
